-- ---------------------------------------------------------------------------
-- Guest-checkout payment confirmation (idempotent, paste + run in the SQL
-- editor; already folded into 0001_init.sql for fresh databases).
--
-- Why: `orders` RLS lets a signed-in customer (or admin) read their rows, so
-- order creation used `insert ... returning` to hand the DB row back. Guests
-- (user_id = null) can INSERT but their rows are invisible to SELECT under
-- RLS — PostgREST surfaced that as "new row violates row-level security policy
-- for table 'orders'" and the payment-verify route could not read guest orders
-- either.
--
-- Fix: the app now generates the order UUID itself and does not read rows
-- back through RLS. `confirm_payment` gains p_order_id (tie the client's order
-- to the Razorpay order id) and RETURNS the updated row as jsonb, so the
-- verify route gets the authoritative row via this security-definer function
-- instead of an RLS-filtered SELECT.
-- ---------------------------------------------------------------------------
